...
import logging
import sys
sys.path.append('/Users/zhehaoli/Documents/Projects/Selective-Labels/Experiment')
        ...
        for confounding_strength_D in confounding_strength_values:
            ...
            for iter in range(num_iterations):
                ...
                data = generate_data(n_samples=10000, n_observed=6, n_unobserved=4, num_iv_levels=5, num_y_classes=3, 
                                     confounding_type='uc', confounding_strength_Y=confounding_strength_Y, confounding_strength_D=confounding_strength_D, 
                                     random_seed=20*iter)
            ...
            # with open(f'UC-Data-Iterations-{num_iterations}-Confounding-Y{confounding_strength_Y}-D{confounding_strength_D}.pickle', 'wb') as handle:
            #     pickle.dump(data_batches, handle, protocol=pickle.HIGHEST_PROTOCOL)

            with open(f'UC-Accuracy-Iterations-{num_iterations}-Confounding-Y{confounding_strength_Y}-D{confounding_strength_D}.pickle', 'wb') as handle:
