# Counterfactual Debias Machine Learning
# Author: Zhehao Li
# Email: lizh21@mails.tsinghua.edu.cn
...
#     # Estimate the propensity score P(D = 1 | X)
#     Model_D = NuisanceEstimation(Sample=Sample_1, features=features, label=decision, instrument=None, ztype='weighted')

#     # Estimate the inverse propesnity weight function given X of Sample 2
#     X = Sample_2[features].values
#     W_IPW = 1 / Model_D.predict_proba(X)[:, 1]
    
#     # Construct new sample
#     Sample_IPW = Sample_2.copy()
#     Sample_IPW['Weight'] = W_IPW

#     # Cutoff the extreme value
#     cutoff_value = Sample_IPW['Weight'].quantile(q=0.9)
#     Sample_IPW['W'] = Sample_IPW[outcome].values
#     Sample_IPW['Weight'] = Sample_IPW['Weight'].clip(upper=cutoff_value)
...
# Compute the inverse propensity scores
def InversePropensityWeight(Sample_1, Sample_2, Params, noise_level=0.0):
    """
    Inputs:
        - Sample_1: pd.DataFrame
        - Sample_2: pd.DataFrame
    ...
    # Estimate the inverse propensity weight function given X of Sample 2
    X = Sample_2[features].values
    ...
    Sample_IPW = Sample_2.copy()
    Sample_IPW['W'] = W
    Sample_IPW['Wnoise'] = W * (1 + noise_level * np.random.randn(W.shape[0]))

    # Cutoff the extreme value
    upper_cutoff = Sample_IPW['W'].quantile(q=0.9)
    upper_cutoff_noise = Sample_IPW['Wnoise'].quantile(q=0.9)

    Sample_IPW['W'] = Sample_IPW['W'].clip(upper=upper_cutoff)
    Sample_IPW['Wnoise'] = Sample_IPW['Wnoise'].clip(upper=upper_cutoff_noise)
...
def IVPointWeightBinary(Sample_1, Sample_2, Params, noise_level=0.0):
    """
    Inputs:
        - Sample_1: pd.DataFrame
        - Sample_2: pd.DataFrame
    ...
    Sample_1_augmented['DY']  = Sample_1_augmented[decision] * Sample_1_augmented[outcome]
    Sample_1_augmented['DZY'] = Sample_1_augmented['DZ'] * Sample_1_augmented[outcome]        
    ...
    Models.update({
        ...
        'DY':  NuisanceEstimation(Sample_1_augmented, features, 'DY', instrument, 'weighted'),
        'DZY': NuisanceEstimation(Sample_1_augmented, features, 'DZY', instrument, 'weighted')
    })

    # Compute the numerator: Cov(DY, Z | X) = E[DZY | X] - E[DY | X] * E[Z | X]
    numerator_1 = Models['DZY'].predict_proba(X) @ np.unique(Sample_1_augmented['DZY'])
    numerator_0 = (Models['DY'].predict_proba(X) @ np.unique(Sample_1_augmented['DY'])) * (Models['Z'].predict_proba(X) @ np.unique(Sample_1_augmented[instrument]))
    ...
    W_point_noise = W_point * (1 + noise_level * np.random.randn(W_point.shape[0]))
 
    # Construct the weight function on Sample_2
    PointSample = Sample_2.copy()
    PointSample['W'] = (W_point > 0).astype(int)
    PointSample['Weight'] = np.abs(W_point)

    # Sensitivity analysis with estimation accuracy: input noise
    PointSample['W_noise'] = (W_point_noise > 0).astype(int)
    PointSample['Weight_noise'] = np.abs(W_point_noise)

    # Cutoff extreme value
    upper_cutoff = PointSample['Weight'].quantile(q=0.9)
    upper_cutoff_noise = PointSample['Weight_noise'].quantile(q=0.9)

    PointSample['Weight'] = PointSample['Weight'].clip(upper=upper_cutoff)
    PointSample['Weight_noise'] = PointSample['Weight_noise'].clip(upper=upper_cutoff_noise)
...
def IVPointWeightMulti(Sample_1, Sample_2, Params, noise_level=0.0):
    """
    Inputs:
        - Sample_1: pd.DataFrame
        - Sample_2: pd.DataFrame
    ...
    Model_DZ = Models['DZ'].predict_proba(X) * (1 + noise_level * np.random.randn())
    Model_D = Models['D'].predict_proba(X) * (1 + noise_level * np.random.randn())
    Model_Z = Models['Z'].predict_proba(X) * (1 + noise_level * np.random.randn())
    denominator_1 = Model_DZ @ np.unique(Sample_1_augmented['DZ'])
    denominator_0 = (Model_D @ np.unique(Sample_1_augmented[decision])) * (Model_Z @ np.unique(Sample_1_augmented[instrument]))
    ...
    for k in Y_vals:
        ...
        Model_DZYk = Models[f'DZY{k}'].predict_proba(X) * (1 + noise_level * np.random.randn())
        Model_DYk  = Models[f'DY{k}'].predict_proba(X)  * (1 + noise_level * np.random.randn())
        numerator_1 = Model_DZYk @ np.unique(Sample_1_augmented[f'DZY{k}'])
        numerator_0 = (Model_DYk @ np.unique(Sample_1_augmented[f'DY{k}'])) * (Model_Z @ np.unique(Sample_1_augmented[instrument]))
    ...
    for k in Y_vals:
        ...
        # Sensitivity analysis with estimation accuracy: input noise
        # PointSample[f'Wnoise_{k}'] = PointSample[f'W_{k}'] * (1 + noise_level * np.random.randn( PointSample[f'W_{k}'].shape[0] ))
        
        # Cutoff extreme value
        upper_cutoff = PointSample[f'W_{k}'].quantile(q=0.9)
        PointSample[f'W_{k}'] = PointSample[f'W_{k}'].clip(upper=upper_cutoff)

        # upper_cutoff_noise = PointSample[f'Wnoise_{k}'].quantile(q=0.9)
        # PointSample[f'Wnoise_{k}'] = PointSample[f'Wnoise_{k}'].clip(upper=upper_cutoff_noise)
...
def IVPartialWeightBinary(Sample_1, Sample_2, Params, ztype='robust', alph=0.0, beta=1.0, noise_level=0.0):
    """
    Inputs:
        - Sample_1: pd.DataFrame
        - Sample_2: pd.DataFrame
    ...
    if ztype == 'pointwise':
        ...
        Models = {
            'D': NuisanceEstimation(Sample_1, features, decision, instrument, 'pointwise'),
        ...
        # Estimates the conditional probability functions
        Models = {
            'D': NuisanceEstimation(Sample_1, features, decision, instrument, 'robust'),
    ...
    # Compute the cost-sensitive weight
    W_partial = np.array(np.abs(U) * (U > 0).astype(int) - np.abs(L) * (L < 0).astype(int))

    # Sensitivity analysis with estimation accuracy: input noise
    # W_partial_noise = W_partial * (1 + noise_level * np.random.randn( W_partial.shape[0] ))

    # Construct the cost-sensitive weight on Sample_2
    PartialSample = Sample_2.copy()
    PartialSample['W'] = (W_partial > 0).astype(int)
    PartialSample['Weight'] = np.abs(W_partial)

    # Corrupted cost-sensitive weight
    # PartialSample['W_noise'] = (W_partial_noise > 0).astype(int)
    # PartialSample['Weight_noise'] = np.abs(W_partial_noise)
...
def IVPartialWeightMulti(Sample_1, Sample_2, Params, ztype='robust', alph=0.0, beta=1.0, noise_level=0.0):
    """
    Inputs:
        - Sample_1: pd.DataFrame
        - Sample_2: pd.DataFrame
    ...
    for k in y_vals:
        ...
        for z in z_vals:
            ...
            # predict_proba returns (n_samples, n_classes) array, classes are order as np.unique(y)
            if ztype == 'pointwise':
                Additive = Models['D'][f'z={z}'].predict_proba(X)[:, 0] * (1 + noise_level * np.random.randn())
                Cond_mean = Models[f'DY{k}'][f'z={z}'].predict_proba(X) * (1 + noise_level * np.random.randn())
                Lbounds[f'z={z}'] = Cond_mean @ np.unique(Sample_1_augmented[f'DY{k}']) + alph * Additive
                Ubounds[f'z={z}'] = Lbounds[f'z={z}'] + (beta - alph) * Additive
                ...
                # Additive = 1 - (Models['D'].predict_proba(Xz) @ np.unique(Sample_1_augmented[decision]))
                Additive = Models['D'].predict_proba(Xz)[:, 0] * (1 + noise_level * np.random.randn())
                Cond_mean = Models[f'DY{k}'].predict_proba(Xz) * (1 + noise_level * np.random.randn())
                Lbounds[f'z={z}'] = Cond_mean  @ np.unique(Sample_1_augmented[f'DY{k}']) + alph * Additive
                Ubounds[f'z={z}'] = Lbounds[f'z={z}'] + (beta - alph) * Additive
    ...
    for k in y_vals:
        ...
        # Sensitivity analysis with estimation accuracy: input noise
        # Width_k_noise = Width_k * (1 + noise_level * np.random.randn( Width_k.shape[0], Width_k.shape[1] ))

        # Construct the final cost-sensitive weight
        PartialSample[f'W_{k}'] = np.maximum(np.max(Width_k, axis=1), 0)
        # PartialSample[f'Wnoise_{k}'] = np.maximum(np.max(Width_k_noise, axis=1), 0)
...
def CrossFitting(Sample, Params, WeightFunc, k_folds=5, random_state=0, noise_level=0):
    """
        Inputs:
            - Train: pd.DataFrame
            - Parms: dictionary
    ...
    for major_idx, minor_idx in kf.split(Sample):
        ...
        Sample_Lst.append( WeightFunc(Sample_1=Sample_major, 
                                      Sample_2=Sample_minor, 
                                      Params=Params, 
...
def CostSensitiveClassificationNN(Sample, Params, WeightFunc, surrogate='exploss', k_folds=5, random_state=0, noise_level=0):
    ...
    # Construct the weighted training set via cross-fitting
    Train_weighted = CrossFitting(Sample=Train, Params=Params, WeightFunc=WeightFunc, 
                                  k_folds=k_folds, random_state=random_state, noise_level=noise_level)
